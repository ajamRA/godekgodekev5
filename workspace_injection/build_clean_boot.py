...
# Do not overwrite the existing candidate with an incomplete SAR repair.
...
def make_support_entry(template_hdr, name, data):
    """Place support files in the normal-boot subtree, not recovery root."""
    if name not in ("p", "w"):
        raise ValueError("Unsupported injection file")
    path = "first_stage_ramdisk/" + name
...
def patch_init_binary(init_raw: bytes) -> bytes:
    ...
    # Native init string kept intact (no redirection)
    assert init_mod[0x136E4 : 0x136E4 + 24] == b"/system/etc/prop.default", "0x136E4 corrupted!"
    print("[+] Verified /system/etc/prop.default at 0x136E4 kept intact.")
    # Verify original instruction at 0x6b6c8: b.ne #0x6b6d8 (0x54000081)
    ...
    assert set(init_mod[0xA9F50:0xAA000]) == {0}, "0xA9F50 cave is not clean zeroes"
    print("[+] Clean init binary: no assembly cave hook, native init logic intact.")
...
"""
    ...
    print("[+] Skipping p and w files: using native debug_ramdisk/adb_debug.prop.")
    # Inject force_debuggable and debug_ramdisk/adb_debug.prop
    hdr_fd = make_file_hdr(template_hdr, 0, len(b"force_debuggable\x00"))
    entries.insert(2, {"hdr": hdr_fd, "name": "force_debuggable", "data": b""})
